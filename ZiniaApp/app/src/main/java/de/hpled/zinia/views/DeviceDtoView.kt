...
class DeviceDtoView(c: Context, attr: AttributeSet?) : LinearLayout(c, attr) {
    ...
    fun setDeviceDto(dto: DeviceStatusDTO) {
...
class DeviceDtoViewAdapter(private val context: Context) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        if(convertView != null && convertView is DeviceDtoView) {
            convertView.setDeviceDto(dto)
            return convertView
        }
        else {
            val view = DeviceDtoView(context, null)
            view.setDeviceDto(dto)
            return view
        }
    }
